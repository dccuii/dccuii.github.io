	...
	@proc_keywords["SGPLOT"] ||= Set.new %w(
      ...
      state :root do
        ...
        rule %r!(proc)(\s+)(\w+)!ix do |m|
          ...
          groups Keyword::Declaration, Text, Keyword::Declaration
          push :proc
        end

        # Data step definitions
        rule %r/(data)(\s+)([\w\.]+)/i do
          groups Keyword::Declaration, Text, Name::Variable
        end
        # Libname definitions
        ...
        # RUN, QUIT definition
        rule %r/\brun\b/i, Keyword::Declaration
        rule %r/\bquit\b/i, Keyword::Declaration
